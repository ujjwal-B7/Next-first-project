...
const Prompt = ({ data, handleTagClick, handleEdit, handleDelete }) => {
  ...
  return (
    <div className="secondary-bg p-4 rounded flex-1 relative">
      {isCopied && (
        <div
          className="secondary-bg absolute -top-9 p-2 right-0 text-xs 
      ...
      <span
        className="primary-bg flex w-[6.4rem] p-1 gap-2 rounded ml-auto mb-4 cursor-pointer"
        onClick={() => copyPrompt(data.prompt)}
      >
        <p className="text-xs third-text">copy prompt</p>
        <svg
      ...
      <Link className="text-blue-400 underline underline-offset-2" href={``}>
        Edit
      </Link>
      <button
